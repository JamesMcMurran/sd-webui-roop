...
class FaceSwapScript(scripts.Script):
    ...
    def ui(self, is_img2img):
        with gr.Accordion(f"roop {version_flag}", open=False):
            with gr.Column():
                with gr.Tab("Face 1"):
                    enable1 = gr.Checkbox(False, placeholder="enable", label="Enable Picture 1")
                    faces_index = gr.Textbox(
                            value="0",
                            placeholder="For face1: Which face to swap (comma separated), start from 0",
                            label="For face1: Comma separated face number(s)",
                        )
                    img = gr.inputs.Image(type="pil")
                with gr.Tab("Face 2"):
                    enable2 = gr.Checkbox(False, placeholder="enable", label="Enable Picture 2")
                    faces_index2 = gr.Textbox(
                            value="0",
                            placeholder="For face2: Which face to swap (comma separated), start from 0",
                            label="For face2: Comma separated face number(s)",
                        )
                    img2 = gr.Image()
                with gr.Tab("Face 3"):
                    enable3 = gr.Checkbox(False, placeholder="enable", label="Enable Picture 3")
                    faces_index3 = gr.Textbox(
                            value="0",
                            placeholder="For face3: Which face to swap (comma separated), start from 0",
                            label="For face3: Comma separated face number(s)",
                        )
                    img3 = gr.Image()
                with gr.Tab("Face 4"):
                    enable4 = gr.Checkbox(False, placeholder="enable", label="Enable Picture 4")
                    faces_index4 = gr.Textbox(
                            value="0",
                            placeholder="For face4: Which face to swap (comma separated), start from 0",
                            label="For face4: Comma separated face number(s)",
                        )
                    img4 = gr.Image()

                enable = gr.Checkbox(False, placeholder="enable", label="Enable Face Swaper")
    ...
    def process(
        ...
        self.faces_index2 = {
            int(x) for x in faces_index2.strip(",").split(",") if x.isnumeric()
        }
        if len(self.faces_index) == 0:
            self.faces_index = {0}
        if self.enable:
            if self.source is not None and self.enable1:
            ...
            if self.source is not None and self.enable2:
                if isinstance(p, StableDiffusionProcessingImg2Img) and swap_in_source:
                    logger.info(f"roop enabled, face index %s", self.faces_index)
    ...
    def postprocess_image(self, p, script_pp: scripts.PostprocessImageArgs, *args):
        if self.enable and self.enable1 and self.swap_in_generated:
        ...
        if self.enable2 and self.enable and self.swap_in_generated:
            if self.source is not None:
                image: Image.Image = script_pp.image
                result: ImageResult = swap_face(
                    self.source2,
